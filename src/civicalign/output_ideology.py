...
median of their cutpoints is the committee's Output Ideology. Its distance from
the Senate median and from the public's centre is the drift.
...
def output_ideology(rollcalls: list[RollCall], bills: dict[str, Bill],
                    chamber_median: float, public_centre: float | None,
                    ) -> list[OutputIdeology]:
    ...
    for code, cps in by_cmte.items():
        ...
        out.append(OutputIdeology(
            ...
            vs_public=(m - public_centre) if public_centre is not None else None))
