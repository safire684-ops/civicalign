"""Pillar 4 as published: a senator against same-party senators from other
states with a similar recent presidential vote.
...
Peers of a senator are senators who
  1. belong to the same party group (Republicans; or Democrats together with the
...
narrowest window at which almost every senator has six same-party peers from
...
def group(party: str) -> str:
    """Republicans form one group; Democrats and the Independents who caucus
    with them form the other."""
    return "Republican" if party == "Republican" else "Democratic"
    ...
    for b, sen, score, x in people:
        ...
        def peers_at(w):
            ps = [Peer(pb, ps_.name, ps_.state, pv) for pb, ps_, pv, px in people
                  if ps_.state != sen.state and group(ps_.party) == g and abs(px - x) <= w + 1e-12]
